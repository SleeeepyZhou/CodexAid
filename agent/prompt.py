...
- 动作合并：对需“同时”或“原子级并行”执行的动作，设计为单个复合工具。
- Dev开发指令：为每个工具补充详细的开发任务说明，并发保障、边界条件和容错方案。
...
{{
    "tools": [
        {{
            ...
            "input": [],
            "output": ["light_on_status", "pomodoro_started"],
...
DEVELOPER = '''
你是一个代码工程师，你需要严格按照要求实现代码，工具代码必须是【单个】函数进行实现，函数必须是独立无需中途额外输入的。
要求如下：
{task}
